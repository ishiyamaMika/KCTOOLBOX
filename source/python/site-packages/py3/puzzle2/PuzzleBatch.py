...
class PuzzleBatch(Puzzle.Puzzle):
    def __init__(self, name="puzzle", **kwargs):
        ...
        print("logger {}:".format(log_directory))
        super(PuzzleBatch, self).__init__(**kwargs)

        self.result_path = os.environ.get("PUZZLE_RESULT_PATH", False)

    def start(self):
        pz_path = os.environ["PUZZLE_TASK_SET_PATH"]
        keys = os.environ.get("PUZZLE_TASK_KEYS", "")
    ...
    def close_event(self, app, messages):
        ...
        if self.result_path:
            ...
            for k, v in self.context.items():
                if k.startswith("_"):
                    extend_info[k] = v
            pz_config.save(self.result_path, messages, extend_info=extend_info)
...
def run_process(app, **kwargs):
    ...
    if isinstance(task_set, dict):
        kwargs["task_set_path"] = "{}/{}_tasks.json".format(pz_env.get_temp_directory(subdir="puzzle/tasks"), now)
    ...
    env_copy["PUZZLE_TASK_SET_PATH"] = str(kwargs["task_set_path"])
    env_copy["PUZZLE_LOGGER_DIRECTORY"] = str(kwargs.get("log_directory", pz_env.get_temp_directory("puzzle/log")))
    env_copy["PUZZLE_LOGGER_NAME"] = str(kwargs["log_name"])
    env_copy["PUZZLE_TASK_KEYS"] = str(kwargs.get("keys", ""))
    env_copy["PUZZLE_APP"] = str(app)

    """
    puzzle path will append if not exists.
    sys path can set list(useful) or string.so, if sys_path is list, convert to string.
    """
    sys_path = kwargs.get("sys_path", "")
    if isinstance(sys_path, list):
        sys_path = ";".join(sys_path)

    puzzle_directory = os.path.dirname(pz_env.get_puzzle_path())
    if puzzle_directory not in sys_path.split(";"):
        sys_path += ";{}".format(puzzle_directory)
    
    env_copy["PUZZLE_SYS_PATH"] = sys_path

    env_copy["PUZZLE_MODULE_DIRECTORY"] = str(kwargs["module_directory_path"])
    env_copy["PUZZLE_CLOSE_APP"] = str(kwargs["close_app"])
    if app == "maya":
        env_copy["MAYA_NO_WARNING_FOR_MISSING_DEFAULT_RENDERER"] = 1
    ...
    if kwargs.get("bat_file") is None:
        ...
        if "bat_file" in kwargs:
            ...
            if kwargs.get("bat_start", False):
                process = subprocess.Popen(kwargs["bat_file"], 
                                           stdin=subprocess.PIPE, 
                                           stdout=subprocess.PIPE, 
                                           stderr=subprocess.PIPE, 
                                           shell=False)
