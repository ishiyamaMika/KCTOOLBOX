...
class Puzzle(object):
    ...
    def play(self, tasks, data_set):
        ...
        def _execute_task(task, data):
            ...
            self.logger.details.set_header("successed: {}".format(task["name"]))
            ...
            try:
                ...
                self.logger.details.set_header("import error: {}.py".format(module_path.split(".")[-1]))
                self.logger.details.add_detail(error)
                self.logger.details.update_code(4)
            ...
            try:
                ...
                self.logger.critical(error)
                self.logger.details.add_detail(error)
                self.logger.details.set_header("execute error: {}.py".format(module_path.split(".")[-1]))
                self.logger.details.update_code(4)
                return {"return_code": 4}
