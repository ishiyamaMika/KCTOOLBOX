...
class Puzzle(object):
    ...
    def play(self, tasks, data_set, default_context={}):
        ...
        if tasks[0]["step"] == "init":
            ...
            self.logger.debug("- init start - {}".format(tasks[0].get("comment", "")))
        ...
        for task in tasks:
            ...
            self.logger.debug("- {} start - {}".format(step, task.get("comment", "")))
        ...
        if tasks[-1]["step"] == "closure":
            self.break_ = False
            self.logger.debug("- closure start - {}".format(tasks[-1].get("comment", "")))
            now = datetime.datetime.now()
