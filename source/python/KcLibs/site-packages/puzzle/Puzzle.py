...
class Puzzle(object):
    def __init__(self, name="puzzle", file_mode=False, **args):
        ...
        if self.file_mode:
            self.messages = self.play_as_file_mode()
    ...
    def play_as_file_mode(self):
        ...
        order = os.environ.get("__PUZZLE_ORDER__", False)
        if order:
            self.order = [l.strip() for l in order.split(",")]
        if pieces_directory:
            if pieces_directory not in sys.path:
                sys.path.append(pieces_directory)
        ...
        if pass_path != "":
            pass_info, self.pass_data = pz_config.read(pass_path)
        else:
            self.pass_data = None
    ...
    def fit(self, piece_data, data, pass_data):
        ...
        try:
            ...
            return False, pass_data, u"[error] {}".format(hook_name), traceback.format_exc()
