...
class FieldValueGenerator(object):
    ...
    def get_field_value(self, template, value):
        template = os.path.normpath(template).replace("\\", "/")
...
class StickyConfig(object):
    ...
    def get_key_file(self, template, directory=None, default="_base_"):
        ...
        for i in range(len(split_template)):
            ...
            if not key_file:
                key_file_ = "%s/%s.yml" % (directory, default)
                if os.path.exists(key_file_):
                    return key_file_

        return key_file

    def get_override_file_list(self, path, default="_base_.yml"):
        if not os.path.exists(path):
            d = os.path.dirname(path)
            return ["{}/{}".format(d, default)]
